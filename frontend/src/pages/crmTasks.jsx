...
const CRMTasks = () => {
  ...
  const handleDeleteAccount = async (accountId) => {
    ...
    try {
      ...
      setTasks(prev => prev.filter(task => task.account !== accountId));
  ...
  }, [authLoading, completionFilter, priorityFilter, addedByFilter]);
  ...
  useEffect(() => {
    if (!authLoading && user?.id && !addedByFilter) {
      setAddedByFilter(String(user.id));
    }
  }, [authLoading, user?.id, addedByFilter]);
  ...
  const resetFiltersToDefault = () => {
    ...
    setAddedByFilter(user?.id ? String(user.id) : 'all');
  };
        ...
        <div className="d-flex flex-wrap align-items-center gap-2">
          ...
          <span className="badge rounded-pill text-bg-light border">
            Added By: {
              addedByFilter === 'all'
                ? 'All'
                : users.find(u => String(u.id) === String(addedByFilter))?.first_name || user?.email || 'Me'
            }
          </span>
        </div>
